...
The offline tarball is self-contained: it carries its launchers, the shared
[sb90-deploy](https://github.com/System-B90/deploy-py) tooling's wheels and
every image. **The bundle's scripts do not exist in this repository** — they
are generated by `sb90-deploy bundle` from `deploy/app.json`.

```bash
mkdir bluz && tar -xvf bluz-offline-vX.Y.Z.tar.gz -C bluz --strip-components=1 && cd bluz
...
sudo apt install -y python3 curl git ca-certificates gnupg
