...
export const CoursesProvider = ({ children }: { children: React.ReactNode; }) =>
{
    ...
    const getCourse = useCallback((id: CourseId): Course | undefined =>
    {
        return courses[ id ];
    }, [ courses ]);
    ...
    {
        apiGetCourses().then((fetchedCourses) =>
        {
        ...
        }).catch((error) => enqueueApiErrorSnackbar(enqueueSnackbar, 'טעינת קורסים נכשלה.', error));
    }, [ setCourses ]);
