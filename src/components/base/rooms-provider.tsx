...
{
    const [ customRoomLookup, setCustomRoomLookup ] = useState<Record<string, CustomRoom>>({});
    const [ hiveRoomLookup, setHiveRoomLookup ] = useState<Record<number, HiveRoom>>({});

    const rooms = useMemo(() => [ ...Object.values(customRoomLookup), ...Object.values(hiveRoomLookup) ], [ customRoomLookup, hiveRoomLookup ]);

    const getRoom = useCallback((id: RoomLike) =>
    {
        if (!(id instanceof Object)) { return null; }
        switch (id.source)
        {
