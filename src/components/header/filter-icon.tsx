    ...
    return (
        <Tooltip title={ `${filtersVisible ? 'Hide' : 'Show'} Filters` } placement='bottom'>
            <IconButton className='relative' color="inherit" onClick={ () => { setFiltersVisible(v => !v); } }>
                ...
                { (!filtersVisible && hasAnyFilter) && <Tooltip placement='right' title='יש סננים נסתרים'><InfoIcon className='absolute top-0.5 right-0.5 opacity-80' fontSize='inherit' sx={ { fontSize: '1.1rem' } } /></Tooltip> }
            </IconButton>
        </Tooltip>
    );
}