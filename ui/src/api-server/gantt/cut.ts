...
import { ApiCurriculumCutError, ApiCurriculumCutResponse } from "@/api-shared/types/gantt/cut";
...
export function buildCutPlanInput(args: {
    ...
    const { curriculum, mappings, exceptions, dayStartTime } = args;
    ...
    const weeks = weekLinks.map((wLink) => {
        ...
        return { id: wLink.week.id, dayIds };
    });
