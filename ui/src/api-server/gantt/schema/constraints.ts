...
export const constraintTypeEnumSchema = pgEnum("constraint_type", [ "RELATIONAL", "TEMPORAL" ]);
export const relationTypeEnumSchema = pgEnum("relation_type", [ "after", "before" ]);

export const ganttConstraintsSchema = pgTable("cntrs", {
    id: text("id").primaryKey(),
    type: constraintTypeEnumSchema("type").notNull(),

    ownerEventId: text("owner_event_id").references(() => ganttEventsSchema.id, { onDelete: "cascade" }),
    ownerModuleId: text("owner_module_id").references(() => ganttModulesSchema.id, { onDelete: "cascade" }),

    relation: relationTypeEnumSchema("relation"),
    targetEventId: text("target_event_id").references(() => ganttEventsSchema.id, { onDelete: "cascade" }),
    targetModuleId: text("target_module_id").references(() => ganttModulesSchema.id, { onDelete: "cascade" }),
