...
export const ganttCurriculum2SyllabusesSchema = pgTable('c2s', {
    curriculumId: text('curriculum_id').notNull().references(() => ganttCurriculumsSchema.id, { onDelete: 'cascade' }),
    syllabusId: text('syllabus_id').notNull().references(() => ganttSyllabusesSchema.id, { onDelete: 'cascade' }),
}, (t) => ({
    pk: primaryKey({ columns: [ t.curriculumId, t.syllabusId ] })
}));
export const ganttCurriculum2SyllabusesRelationsSchema = relations(ganttCurriculum2SyllabusesSchema, ({ one }) => ({
...
export const ganttSyllabus2ModulesSchema = pgTable('s2m', {
    syllabusId: text('syllabus_id').notNull().references(() => ganttSyllabusesSchema.id, { onDelete: 'cascade' }),
    moduleId: text('module_id').notNull().references(() => ganttModulesSchema.id, { onDelete: 'cascade' }),
}, (t) => ({
    pk: primaryKey({ columns: [ t.syllabusId, t.moduleId ] })
}));

export const ganttSyllabus2ModulesRelationsSchema = relations(ganttSyllabus2ModulesSchema, ({ one }) => ({
    syllabus: one(ganttSyllabusesSchema, {
...
export const ganttModule2EventsSchema = pgTable('m2e', {
    moduleId: text('module_id').notNull().references(() => ganttModulesSchema.id, { onDelete: 'cascade' }),
...
export const ganttModule2EventsRelationsSchema = relations(ganttModule2EventsSchema, ({ one }) => ({
    module: one(ganttModulesSchema, {
...
export const ganttCurriculum2WeeksSchema = pgTable('c2w', {
    curriculumId: text('c_id').notNull().references(() => ganttCurriculumsSchema.id, { onDelete: 'cascade' }),
    weekId: text('w_id').notNull().references(() => ganttWeeksSchema.id, { onDelete: 'cascade' }),
}, (t) => ({
    pk: primaryKey({ columns: [ t.curriculumId, t.weekId ] })
}));
export const ganttCurriculum2WeeksRelationsSchema = relations(ganttCurriculum2WeeksSchema, ({ one }) => ({
    curriculum: one(ganttCurriculumsSchema, {
...
export const ganttWeek2DaysSchema = pgTable('w2d', {
    weekId: text('w_id').notNull().references(() => ganttWeeksSchema.id, { onDelete: 'cascade' }),
    dayId: text('d_id').notNull().references(() => ganttDaysSchema.id, { onDelete: 'cascade' }),
}, (t) => ({
    pk: primaryKey({ columns: [ t.weekId, t.dayId ] })
}));

export const ganttWeek2DaysRelationsSchema = relations(ganttWeek2DaysSchema, ({ one }) => ({
    week: one(ganttWeeksSchema, {
