...
const DAYS_OF_WEEK = [
    ...
    { index: GanttDayIndex.Monday, label: getDayNameDisplay(GanttDayIndex.Monday), short: 'ב' },
    { index: GanttDayIndex.Tuesday, label: getDayNameDisplay(GanttDayIndex.Tuesday), short: 'ג' },
    { index: GanttDayIndex.Wednesday, label: getDayNameDisplay(GanttDayIndex.Wednesday), short: 'ד' },
    { index: GanttDayIndex.Thursday, label: getDayNameDisplay(GanttDayIndex.Thursday), short: 'ה' },
    { index: GanttDayIndex.Friday, label: getDayNameDisplay(GanttDayIndex.Friday), short: 'ו' },
    { index: GanttDayIndex.Saturday, label: getDayNameDisplay(GanttDayIndex.Saturday), short: 'ש' },
] as const;
...
export const WeekDayVisualizer: React.FC<WeekDayVisualizerProps> = ({ validDays }) =>
{
    return (
...
export function TemporalDraftFields({ draft, setDraft }: TemporalDraftFieldsProps)
{
    const validDays = useMemo(() =>
    {
        const allowed = draft.allowedDays && draft.allowedDays.length > 0
    ...
    const allowedValues = useMemo(() =>
        DAYS_OF_WEEK.filter(d => (draft.allowedDays || []).includes(d.index as GanttDayIndex)),
    [ draft.allowedDays ]);

    const forbiddenValues = useMemo(() =>
        DAYS_OF_WEEK.filter(d => (draft.forbiddenDays || []).includes(d.index as GanttDayIndex)),
    [ draft.forbiddenDays ]);

    const handleAllowedChange = (_event: React.SyntheticEvent, newValue: Array<typeof DAYS_OF_WEEK[ number ]>) =>
    {
        setDraft({
            ...draft,
    ...
    const handleForbiddenChange = (_event: React.SyntheticEvent, newValue: Array<typeof DAYS_OF_WEEK[ number ]>) =>
    {
        setDraft({
            ...draft,
        ...
        <Stack direction='row' spacing={ 3 }>
            <Stack alignItems="flex-start" direction="row" spacing={ 2 }>
                <Autocomplete
                    ...
                    renderInput={ (params) => (
                        <TextField { ...params } label="מותר שיהיה בימי..." placeholder="בחרו ימים" />
                    ) }
                    size="small"
                    sx={ { width: 250 } }
                    value={ allowedValues }
                ...
                <Autocomplete
                    ...
                    options={ DAYS_OF_WEEK }
                    renderInput={ (params) => (
                        <TextField { ...params } label="אסור שיהיה בימי..." placeholder="בחרו ימים" />
                    ) }
                    size="small"
                    sx={ { width: 250 } }
