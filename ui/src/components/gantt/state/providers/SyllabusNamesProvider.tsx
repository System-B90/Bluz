...
const SyllabusContext = createContext<SyllabusProviderState | undefined>(undefined);

export function SyllabusNamesProvider({ children }: { children: React.ReactNode; })
{
    ...
    const fetchSyllabuses = useCallback(async () =>
    {
        ...
        {
            setError(err instanceof Error ? err : new Error("Failed to fetch syllabuses"));
            enqueueApiErrorSnackbar(enqueueSnackbar, 'טעינת שמות הסילבוסים נכשלה!', err);
        } finally
        {
            setIsLoading(false);
        }
...
export function useSyllabusNames(): SyllabusProviderState
{
    ...
    {
        throw new Error("useSyllabusNames must be used within a SyllabusNamesProvider");
    }
    return context;
}
