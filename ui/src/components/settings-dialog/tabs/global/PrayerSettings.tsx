...
export function PrayerSettings() {
    ...
    const getInitialTimes = useCallback(
        () => ({
            shacharit: prayerTimes?.shacharit
                ? dayjs(prayerTimes.shacharit)
                : dayjs().hour(6).minute(0),
            mincha: prayerTimes?.mincha
                ? dayjs(prayerTimes.mincha)
                : dayjs().hour(12).minute(0),
            arvit: prayerTimes?.arvit
                ? dayjs(prayerTimes.arvit)
    ...
    const handleTimeChange = useCallback(
        (key: keyof typeof localTimes, newValue: Dayjs | null) => {
            setLocalTimes((prev) => ({
                ...prev,
                [key]: newValue,
            }));
