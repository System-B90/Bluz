...
export const calendarReducer = (state: Array<Event>, action: CalendarAction): Array<Event> =>
{
    switch (action.type)
    {
    case "SET_EVENTS":
...
{
    const { state: events, set: setEvents, undo, redo } = useHistoryState<Array<Event>>(initialState);
