        ...
        <Box { ...props }>
            {
                getPresentInstructors(event).length === 0 ?
                    <Box alignItems={ 'center' } display={ 'flex' } flexDirection={ 'row' }>
                        ...
                        <Typography color='error' fontWeight={ 600 } variant="caption">אין מבוזרים</Typography>
                    </Box> :
                    <>
                        { showCaption ? <Typography fontWeight={ 600 } marginTop={ 0 } noWrap paddingBottom={ 0 } variant="caption">{ event.instructors.length === 1 ? 'מבוזר' : 'מבוזרים' }</Typography> : null }
                        <Stack direction={ (props.flexDirection === 'column') ? "column" : "row" } display={ 'flex' } flexWrap={ 'wrap' } gap={ 0.3 } sx={ { marginTop: '0 !important' } }>
                            {
