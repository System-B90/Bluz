...
test.describe("Gantt Page", () => {
  ...
  test("renders the Gantt page with placeholder text", async ({ page }) => {
    ...
    await expect(page.getByText("בחרו גאנט כדי להתחיל לעבוד")).toBeVisible();
  });
  ...
  test("selects a curriculum and shows loading/content", async ({ page }) => {
    ...
    if ((await curriculumItems.count()) > 0) {
      ...
      const loadingOrContent =
        ...
        (await page.getByText("בחרו גאנט כדי להתחיל לעבוד").count()) === 0;
  ...
  test("URL cid parameter syncs with selected curriculum", async ({ page }) => {
  ...
  test("navigates back to schedule from Gantt via AppBar", async ({ page }) => {
    const appBar = page.locator(SELECTORS.appBar);
